...
export function simplifyTypeName(typeName: string | null): string {
	if (!typeName) return '';

	// if typename has (), that means that this function returns an empty value
	if (typeName.includes('()')) return '()';

	let simplified = typeName
		// Remove leading namespace, keep only the last namespaces
		.replace(/(\w+::)+(?=\w+)/g, '')
		.replace(/<([^<>]*)>/g, (_, inside) => {
			// Edge case: if there's parentheses with a trailing comma, clean it
			// NOTE: This case exists as sierra ContractClass DebugInfo return for example, type_name": "core::panics::PanicResult::<(core::bool,)>"
			let regexBracketSpaces = /^\(\s*(.*?)\s*,*\s*\),*$/;
			let match = inside.trim().match(regexBracketSpaces);
			if (match) {
				inside = match[1].trim();
			}
			// Remove leading namespaces inside the generics
			return '<' + inside.replace(/(\w+::)+/g, '') + '>';
		});

	// Handle specific case for cleaner view to remove <, ( from the start and end, in case there is no namespace
	let regexSourrounding = /^<(\(?)(.*?)(\)?)>$/;
	const match = simplified.match(regexSourrounding);
	if (match) {
		return match[2].trim();
	}
