...
const EventsPage = ({ params }: { params: { project_id: string, error_id: string } }) => {
    ...
    const getMonitoringEvents = async (page: number) => {
        ...
        const monitoringEvents = await getMonitoringEventsApi(params.project_id, params.error_id, offset);
        setEvents(monitoringEvents);
    };

    const getMonitoringErrorOverview = async () => {
        const overview = await getMonitoringErrorEventsOverviewApi(
            params.project_id, params.error_id
        );
                ...
                <section className="flex flex-col gap-4 px-4 sm:px-6 lg:px-8 py-6">
                    <div className="flex flex-row gap-4">
                        ...
                        <Card className="basis-1/3  ">
                            ...
                            <CardContent className="">
                                <div className="text-2xl font-bold "> {monitoringEventsOverview?.firstOccurrenceDate.toLocaleString()}</div>
                            </CardContent>
                        </Card>
                    </div>
