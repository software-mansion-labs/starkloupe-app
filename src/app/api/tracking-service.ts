export const isTrackingActive = () => {
    ...
    if (shouldSkipSentryBasedOnQueryParam) {
        console.log('Sentry is disabled on client - skip_tracking=true query param is set');
    }
    // Also to disable Sentry on client - set 'skip-sentry-pls=true' cookie
    const cookies = document.cookie.split(';');
    const shouldSkipSentryBasedOnCookie = cookies.some(cookie => cookie.trim().startsWith('skip_tracking_pls=true'));
    if (shouldSkipSentryBasedOnCookie) {
