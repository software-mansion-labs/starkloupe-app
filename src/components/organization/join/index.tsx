...
import { useSetGlobalOrganizationIdInUserContext, useUserContext } from '@/lib/context/user-context-provider';
import { SignUpWithGithubButton } from '@/components/auth/sign-up-with-github-button';
import { toast } from '@/components/hooks/use-toast';
import { acceptInvitationApi, getInvitationDetailsApi, InvitationDetails } from '@/app/api/monitoring-api-service';
...
export function JoinOrganizationByInvitationPage() {
    ...
    const [invitationDetails, setInvitationDetails] = React.useState<InvitationDetails | undefined>(undefined);
    const [buttonDisabled, setButtonDisabled] = React.useState(false);
    const router = useRouter();
    const redirectUri = `${process.env.NEXT_PUBLIC_APP_BASE_URL ?? ''}${process.env.NEXT_PUBLIC_LOGTO_SIGN_UP_INVITATION_REDIRECT_URL}`;
    ...
    const getInvitationDetails = async () => {
        const invitationDetailsRes = await getInvitationDetailsApi(searchParams.get('organizationId') ?? '', searchParams.get('email') ?? '');
    ...
    const acceptInvitation = async () => {
        ...
        const accepted = await acceptInvitationApi(organizationId, invitationDetails?.invitationId ?? '');
        if (accepted) {
                        ...
                        {invitationDetails && (
                            <Card x-chunk="dashboard-04-chunk-1">
                                ...
                                <CardContent className='text-center'>
                                    {isLogged ? (<Button onClick={acceptInvitation} disabled={!invitationDetails || buttonDisabled}>Accept invitation</Button>) : (<SignUpWithGithubButton redirectUri={redirectUri}></SignUpWithGithubButton>)}
                                </CardContent>
                            </Card>
                        )}
                    </div>
