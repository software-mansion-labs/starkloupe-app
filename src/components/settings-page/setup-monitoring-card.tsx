...
export function SetupMonitoringCard({changeTabCallback}: { changeTabCallback: (tab: 'networks' | 'monitoring' | 'members') => void }) {
	...
	const getApiKey = async () => {
		...
		if (apiKeyRes) {
			...
			const chain: Chain = {
				...
				network: apiKeyRes.network === 'CUSTOM' ? networks.find((n) => n.id === apiKeyRes.customNetworkId) : undefined
			};
	...
	const generateApiKey = async () => {
		if (chain) {
			...
			try {
				...
				const apiKeyRes = await createMonitoringApiKeyApi(chainId, organizationIdToUse, chain.network?.id);
				if (apiKeyRes) {
					toast({
					...
					{/*{apiKey !== '' && (*/}
						<div className="flex flex-col gap-6">
							<div className="flex flex-col gap-3">
								...
								<NetworksSelect onChainChangedCallback={onChainChangedCallback} selectedChain={initialChain} />
								<p className="text-muted-foreground text-sm -mt-2">
									If your network is not listed, you can add in&nbsp;
									<Link onClick={() => changeTabCallback('networks')} className="underline" href="/settings">
										custom networks
									</Link>
									.
								</p>
